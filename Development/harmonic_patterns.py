...
def posSize(accountBalance,percentageRisk,pipRisk):
...
def walk_forward(price,sign,slippage=4,stop=10):
    ...
    if sign == 1:
        ...
        for i in range(1,len(price)):
            ...
            elif price[i] < initial_stop_loss:
...
def sup_res_levels(data,p=0.5,delta=0.005):
    ...
    blocks = get_blocks(peak_values, delta)
    ...
    for i in blocks:
        ...
        revs = np.append(len(i), revs)
        stds = np.append(np.std(i),stds)

    revs = (revs - revs.min())/(revs.max()-revs.min())

    if False:
        plt.plot(x, y, label='Original')
        plt.plot(x, y_smooth, label='Smoothed')
        plt.hlines(means, x.min(), x.max())
        plt.scatter(zero_index, y_smooth[zero_index], label="zero derivative")
        plt.legend(loc=0).draggable()
        plt.show()


    return means,stds,revs
